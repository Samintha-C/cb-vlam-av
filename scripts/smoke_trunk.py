"""CPU smoke test for the generation shared trunk (no backbone, no store).

Validates, on random features with a fake 27-concept layout, that the new
generation modules wire together and train:

  1. GradientReversal negates (and scales) the gradient — the core of the
     adversarial disentanglement.
  2. CBL.to_activations builds the normalized concept vector at the expected
     width, and activation_slices covers it exactly.
  3. UnsupervisedResidual + AdversarialDiscriminator + FinalPredictor forward
     in both output modes (regression for continuous-regression, vocab for autoregressive).
  4. A combined loss (L_c + L_t + adversary + sparsity) backpropagates to ALL
     trunk modules, and the adversary's reversed gradient reaches the residual.
...
from cb_vlam.models.adversarial import AdversarialDiscriminator, grad_reverse
...
def main() -> None:
    ...
    # ── 1. GradientReversal sign ──────────────────────────────────────────────
    x = torch.ones(3, requires_grad=True)
    grad_reverse(x, 2.0).sum().backward()
    assert torch.allclose(x.grad, torch.full((3,), -2.0)), \
        f"GRL should negate*lambda; got {x.grad.tolist()}"
    print("GRL: gradient negated and scaled by lambda  ✓")
    ...
    # ── 3. Forward (single-tap, continuous-regression) ────────────────────────────────────────
    feats = torch.randn(B, D)
    cbl_out = cbl(feats)
    cvec = cbl.to_activations(cbl_out)
    r = residual(feats)
    assert cvec.shape == (B, cbl.activation_dim), cvec.shape
    assert r.shape == (B, R), r.shape

    traj = head_reg(cvec, r)
    vocab = head_vocab(cvec, r)
    assert traj.shape == (B, H), traj.shape
    assert vocab.shape == (B, V), vocab.shape
    print(f"forward: cvec{tuple(cvec.shape)} r{tuple(r.shape)} "
          f"-> traj{tuple(traj.shape)} vocab{tuple(vocab.shape)}  ✓")

    # ── 4. Combined loss + backward reaches everything ────────────────────────
    targets = _fake_targets(B)
    Lc = concept_loss(cbl_out, targets)["total"]
    Lt = trajectory_loss(traj, torch.randn(B, H))
    adv_out = adversary(r, lambda_=1.0)
    Ladv = adversary_loss(adv_out, targets)["total"]
    Lreg = elastic_net_penalty(head_reg.concept_weight)
    total = Lc + Lt + Ladv + 1e-3 * Lreg
    assert torch.isfinite(total), total

    total.backward()
    for name, mod in [("cbl", cbl), ("residual", residual),
                      ("adversary", adversary), ("head_reg", head_reg)]:
        g = _grad_norm(mod.parameters())
        assert g > 0, f"{name} received no gradient"
        print(f"  {name:10s} grad norm = {g:.4f}")
    # The adversary's reversed gradient must reach the residual's parameters.
    assert _grad_norm(residual.parameters()) > 0, "residual got no adversarial gradient"
    print("combined L_c + L_t + adversary + sparsity backprops to all modules  ✓")
    ...
    # ── 6. Trajectory parsing + masked L_t + ADE/FDE ──────────────────────────
    ...
    # short parse → tail masked out
    tj2, msk2 = parse_trajectory("formatted as [x, y]: [1.0, 2.0]", horizon=6)
    assert msk2[:2].all() and not msk2[2:].any(), msk2
    pred_t = torch.zeros(1, 12)
    gt_t = torch.from_numpy(tj)[None]
    lt = trajectory_loss(pred_t, gt_t, mask=torch.from_numpy(msk)[None])
    assert torch.isfinite(lt) and lt > 0, lt
    print(f"trajectory: parse→6 wp, masked smooth-L1={lt.item():.3f}  ✓")

    print("\nSMOKE PASSED: generation shared trunk + continuous-regression data path wire together and train.")
